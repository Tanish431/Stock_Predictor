...
function interval_management(date, interval){
  let backdate = new Date(date)
    if (interval=='Weekly'){
      return backdate.setDate(backdate.getDate() - 7)
    }else if (interval=='Month'){
      return backdate.setMonth(backdate.getMonth()-1)
    }else if (interval=='3-Month'){
      return backdate.setMonth(backdate.getMonth() - 3)
    }else if (interval=='6-Month'){
      return backdate.setMonth(backdate.getMonth() - 6)
    }else if (interval=='Year'){
      return backdate.setYear(backdate.getYear() - 1)
    }
}
function sub() {
  const timefrom = ''
  // const timeto = document.getElementById('timeto').value
  const interval = document.getElementById('selected_interval').textContent
    ...
    const dates = rawData.map(function (item) {
      formattedDates=datetoyyyymmdd(item[0]);
    ...
    if (timefrom!=''){
      startDate = new Date(timefrom.replace(/-/g, '/'));
      endDate = new Date(timeto.replace(/-/g, '/'));
    }else{
      endDate = new Date(dates[dates.length-1])
      startDate = new Date(interval_management(endDate, interval))
      ...
      for (let i = 0; i < datesInRange.length; i++){
        if (datesInRange[i] >= startDate.toLocaleDateString('en-CA', { year: 'numeric', month: '2-digit', day: '2-digit' }).replace(/-/g, '/') && datesInRange[i] <= endDate.toLocaleDateString('en-CA', { year: 'numeric', month: '2-digit', day: '2-digit' }).replace(/-/g, '/')){
          dataInRange.push(data[dates.indexOf(datesInRange[i])])
        }
      }
